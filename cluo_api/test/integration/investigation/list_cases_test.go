...
func TestListCases(t *testing.T) {
	...
	t.Run("FilterByClientId", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			...
			case1Encx, err := investigation.ProcessInvestigationEncx(ctx, crypto, case1)
			require.NoError(t, err)
			err = ch.InsertCaseEncx(t, ctx, testPool, case1Encx)
			require.NoError(t, err)
			...
			case2Encx, err := investigation.ProcessInvestigationEncx(ctx, crypto, case2)
			require.NoError(t, err)
			err = ch.InsertCaseEncx(t, ctx, testPool, case2Encx)
			require.NoError(t, err)
		}
	...
	t.Run("FilterByStatus", func(t *testing.T) {
		...
		draftCase.Status = investigation.StatusDraft

		progressCase := ch.NewTestCase(t)
		progressCase.ClientID = clientID
		progressCase.AssignedContactID = nil
		progressCase.Status = investigation.StatusInProgress

		readyCase := ch.NewTestCase(t)
		readyCase.ClientID = clientID
		readyCase.AssignedContactID = nil
		readyCase.Status = investigation.StatusReady

		draftCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, draftCase)
		progressCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, progressCase)
		readyCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, readyCase)
	...
	t.Run("SearchFunctionality", func(t *testing.T) {
		...
		importantCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, importantCase)
		normalCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, normalCase)
		urgentCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, urgentCase)
	...
	t.Run("DateRangeFiltering", func(t *testing.T) {
		...
		oldCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, oldCase)
		...
		recentCaseEncx, _ := investigation.ProcessInvestigationEncx(ctx, crypto, recentCase)
