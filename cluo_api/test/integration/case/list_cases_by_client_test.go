...
func TestListCasesByClient(t *testing.T) {
	...
	t.Run("MultipleClientsIsolation", func(t *testing.T) {
		...
		var response1 caseDomain.ListCasesResponse
		...
		var response2 caseDomain.ListCasesResponse
		...
		var response3 caseDomain.ListCasesResponse
	...
	t.Run("AssignedContactHandling", func(t *testing.T) {
		...
		caseWithContact1Encx, err := caseDomain.ProcessCaseEncx(ctx, crypto, caseWithContact1)
		require.NoError(t, err)
		...
		caseWithoutContactEncx, err := caseDomain.ProcessCaseEncx(ctx, crypto, caseWithoutContact)
		require.NoError(t, err)
		...
		caseWithContact2Encx, err := caseDomain.ProcessCaseEncx(ctx, crypto, caseWithContact2)
		require.NoError(t, err)
...
func findCaseByTitle(cases []*caseDomain.CaseResponse, title string) *caseDomain.CaseResponse {
	for _, c := range cases {
		if c.Title == title {
			return c
		}
