...
ALTER TABLE invoices ADD CONSTRAINT chk_invoices_date_logic
    CHECK (due_date >= issue_date);

ALTER TABLE invoices ADD CONSTRAINT chk_invoices_positive_amounts
    CHECK (
        total_amount >= 0 AND
        tax_amount >= 0 AND
        tax_rate >= 0
...
ALTER TABLE invoices ADD CONSTRAINT chk_invoices_payment_logic
    CHECK (
        (paid_at IS NULL AND paid_amount IS NULL) OR
        (paid_at IS NOT NULL AND paid_amount IS NOT NULL AND paid_amount >= 0)
    );

ALTER TABLE invoices ADD CONSTRAINT chk_invoices_late_fee_logic
    CHECK (
