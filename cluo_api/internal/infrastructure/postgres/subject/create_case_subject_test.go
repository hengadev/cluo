...
func TestCreateCaseSubject(t *testing.T) {
	...
	t.Run("successful creation", func(t *testing.T) {
		...
		err := repo.CreateCaseSubject(ctx, sEncx)
		assert.NoError(t, err, "Failed to create case subject")
	...
	t.Run("nil subject returns error", func(t *testing.T) {
		...
		err := repo.CreateCaseSubject(ctx, nil)
		assert.Error(t, err)
	})
