...
async function generateIcon(size: number, outputPath: string) {
	// Create a simple, clean PWA icon with a dark "C" on white background
	const svg = `
	<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
		...
		<path d="M ${size * 0.62} ${size * 0.22}
		         ...
		         Q ${size * 0.78} ${size * 0.78} ${size * 0.78} ${size * 0.62}
		         L ${size * 0.78} ${size * 0.38}
		         Q ${size * 0.78} ${size * 0.22} ${size * 0.62} ${size * 0.22}"
			  fill="none"
			  stroke="#1a1a1a"
